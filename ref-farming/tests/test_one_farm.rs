...
fn one_farm_whole_lifecycle() {
    ...
    let farmer = root.create_user(
        AccountId::new_unchecked("farmer".to_string()),
        to_yocto("100"),
    );
    ...
    assert_eq!(
        view!(pool.mft_balance_of("0".to_string(), farmer.account_id.clone()))
            .unwrap_json::<U128>()
            .0,
        to_yocto("1")
    );
    ...
    call!(
        root,
        pool.mft_register("0".to_string(), farming_id()),
        deposit = to_yocto("1")
    )
    .assert_success();
    ...
    let user_seeds = show_userseeds(&farming, farmer.account_id().into(), false);
    assert_eq!(
        user_seeds.get(&String::from("swap@0")).unwrap().0,
        to_yocto("1")
    );
    let unclaim = show_unclaim(&farming, farmer.account_id().into(), farm_id.clone(), false);
    assert_eq!(unclaim.0, 0_u128);
    show_seedsinfo(&farming, false);
    println!(
        "----->> Farmer staked liquidity at #{}.",
    ...
    if root.borrow_runtime_mut().produce_blocks(60).is_ok() {
        println!();
        println!(
            "*** Chain goes for 60 blocks *** now height: {}",
            root.borrow_runtime().current_block().block_height,
        );
        let farm_info = show_farminfo(&farming, farm_id.clone(), false);
        assert_farming(
    ...
    if root.borrow_runtime_mut().produce_blocks(540).is_ok() {
        println!();
        println!(
            "*** Chain goes for 60*9 blocks *** now height: {}",
            root.borrow_runtime().current_block().block_height,
        );
        let farm_info = show_farminfo(&farming, farm_id.clone(), false);
        assert_farming(
...
fn one_farm_one_farmer() {
    ...
    let farmer1 = root.create_user(
        AccountId::new_unchecked("farmer1".to_string()),
        to_yocto("100"),
    );
    ...
    assert_eq!(
        view!(pool.mft_balance_of("0".to_string(), farmer1.account_id.clone()))
            .unwrap_json::<U128>()
            .0,
        to_yocto("1")
    );
    ...
    let user_seeds = show_userseeds(&farming, farmer1.account_id().into(), false);
    assert_eq!(
        user_seeds.get(&String::from("swap@0")).unwrap().0,
        to_yocto("1")
    );
    let unclaim = show_unclaim(&farming, farmer1.account_id().into(), farm_id.clone(), false);
    assert_eq!(unclaim.0, 0_u128);
    show_seedsinfo(&farming, false);
    println!(
        "----->> Farmer1 staked liquidity at #{}.",
        root.borrow_runtime().current_block().block_height
    );
    ...
    if root.borrow_runtime_mut().produce_blocks(20).is_ok() {
        println!();
        println!(
            "*** Chain goes for 20 blocks *** now height: {}",
            root.borrow_runtime().current_block().block_height
        );
        let unclaim = show_unclaim(&farming, farmer1.account_id().into(), farm_id.clone(), false);
    ...
    assert_eq!(unclaim.0, 0_u128);
    println!(
        "----->> Farmer1 claimed reward again at #{}.",
        root.borrow_runtime().current_block().block_height
    );
...
fn one_farm_two_farmers() {
    ...
    let farmer1 = root.create_user(AccountId::new_unchecked("farmer1".to_string()), to_yocto("100"));
    let farmer2 = root.create_user(AccountId::new_unchecked("farmer2".to_string()), to_yocto("100"));
    ...
    assert_eq!(
        view!(pool.mft_balance_of("0".to_string(), farmer2.account_id.clone()))
            .unwrap_json::<U128>()
            .0,
        to_yocto("1")
    );
    ...
    assert_eq!(farm_info.claimed_reward.0, 0_u128);
    assert_eq!(farm_info.unclaimed_reward.0, to_yocto("1"));
    let user_seeds = show_userseeds(&farming, farmer2.account_id().into(), false);
    assert_eq!(
        user_seeds.get(&String::from("swap@0")).unwrap().0,
        to_yocto("1")
    );
    let unclaim = show_unclaim(&farming, farmer1.account_id().into(), farm_id.clone(), false);
    assert_eq!(unclaim.0, to_yocto("1"));
    let unclaim = show_unclaim(&farming, farmer2.account_id().into(), farm_id.clone(), false);
    assert_eq!(unclaim.0, 0_u128);
    println!(
        "----->> Farmer2 staked liquidity at #{}.",
        root.borrow_runtime().current_block().block_height
    );
    ...
    let unclaim = show_unclaim(&farming, farmer1.account_id().into(), farm_id.clone(), false);
    assert_eq!(unclaim.0, 0_u128);
    let claimed = show_reward(&farming, farmer1.account_id().into(), dai().into());
    ...
    let out_come = call!(
        farmer1,
        farming.withdraw_reward(dai(), None),
        deposit = 1
    );
    out_come.assert_success();
    println!(
        "----->> Farmer1 withdraw reward at #{}.",
        root.borrow_runtime().current_block().block_height
    );
    let claimed = show_reward(&farming, farmer1.account_id().into(), dai().into());
