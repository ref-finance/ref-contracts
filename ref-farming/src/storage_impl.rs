    ...
    fn storage_deposit(
        ...
        if deposited == 0 {
            ...
            if registration_only {
                self.internal_register_account(
                    &account_id,
                    Contract::suggested_min_storage_usage(),
                );
            ...
            // old account, only can complement storage fee
            if registration_only {
...
impl Contract {
    ...
    pub(crate) fn internal_farmer_storage(&self, account_id: &AccountId) -> (Balance, Balance) {
    ...
    pub(crate) fn assert_storage_usage(&self, account_id: &AccountId) {
        ...
        assert!(locked <= deposited, "{}", ERR11_INSUFFICIENT_STORAGE);
    }
    ...
    pub(crate) fn internal_register_account(&mut self, account_id: &AccountId, amount: Balance) {
        if let Some(mut farmer) = self.get_farmer_wrapped(&account_id) {
            ...
            self.data_mut()
                .farmers
                .insert(&account_id, &VersionedFarmer::new(amount));
            self.data_mut().farmer_count += 1;
        }
    }
}
