...
use crate::errors::*;
use crate::degen_swap::math::{
    Fees, DegenSwap, SwapResult, MAX_AMP, MAX_AMP_CHANGE, MIN_AMP, MIN_RAMP_DURATION,
};
use crate::utils::{add_to_collection, u128_ratio, SwapVolume, FEE_DIVISOR, U256};
use crate::StorageKey;

pub use self::degen::*;
pub use self::price_oracle::*;
pub use self::pyth_oracle::*;
...
impl DegenSwapPool {
    ...
    pub fn get_degens(&self) -> Vec<u128> {
        self.token_account_ids
        .iter()
        .map(|token_id| {
            global_get_degen(token_id).get_price_info().stored_degen
        })
        .collect()
    }
    ...
    pub fn get_amp(&self) -> u64 {
        if let Some(amp) = self.get_invariant_with_degens(&self.get_degens()).compute_amp_factor() {
            amp as u64
        } else {
            0
        }
    ...
    pub fn get_share_price(&self) -> u128 {
        self.get_invariant_with_degens(&self.get_degens())
            .compute_d_with_degens(&self.c_amounts)
    ...
    fn calc_add_liquidity_with_degens(
        ...
        let invariant = self.get_invariant_with_degens(degens);
        ...
        if self.shares_total_supply == 0 {
            ...
            (
                invariant
                    .compute_d_with_degens(&c_amounts)
                    .expect(ERR66_INVARIANT_CALC_ERR)
                    .as_u128(),
                0,
            )
    ...
    pub fn predict_remove_degen_liquidity_by_tokens(
        ...
        let invariant = self.get_invariant_with_degens(degens.as_ref().unwrap_or(&self.get_degens()));
    ...
    pub fn remove_liquidity_by_tokens(
        ...
        let invariant = self.get_invariant_with_degens(&self.get_degens());
        let trade_fee = Fees::new(self.total_fee, &fees);
    ...
    fn internal_get_return(
        ...
        self.assert_degens_valid();
        self.internal_get_return_with_degens(token_in, amount_in, token_out, &self.get_degens(), fees)
    }
    ...
    /// predict swap result with given degen token price
    pub fn get_degen_return(
        ...
        let c_amount_out = self.internal_get_return_with_degens(
    ...
    pub fn ramp_amplification(&mut self, future_amp_factor: u128, future_amp_time: Timestamp) {
        ...
        let amp_factor = self.get_invariant_with_degens(&self.get_degens())
            .compute_amp_factor()
            .expect(ERR66_INVARIANT_CALC_ERR);
        assert!(
...
mod tests {
    ...
    use crate::degen_swap::pyth_oracle::PythOracleConfig;
    use crate::degen_swap::price_oracle::PriceOracleConfig;
    use crate::{price_oracle, pyth_oracle};

    use super::*;

    fn swap(
        pool: &mut DegenSwapPool,
        token_in: usize,
        amount_in: Balance,
        token_out: usize,
    ) -> Balance {
    ...
    fn new_degen_pool(decimals: u8, amp_factor: u128, total_fee: u32) -> DegenSwapPool {
        DegenSwapPool::new(
            0,
    ...
    fn test_mix() {
        ...
        testing_env!(context.block_timestamp(25).predecessor_account_id(accounts(0)).build());
        let fees = AdminFees::zero();
        let mut pool = new_degen_pool(TARGET_DECIMAL, 1000, 0);
        assert_eq!(
            pool.tokens(),
            vec![accounts(1).to_string(), accounts(2).to_string()]
        );

        let cross_call_result1 = near_sdk::serde_json::to_vec(&price_oracle::PriceData{
            timestamp: 20,
            recency_duration_sec: 90,
            prices: vec![price_oracle::AssetOptionalPrice{
                asset_id: accounts(1).to_string(),
                price: Some(price_oracle::Price {
                    multiplier: 20000,
                    decimals: TARGET_DECIMAL + 4,
                }),
            }],
        }).unwrap();
        let cross_call_result2 = near_sdk::serde_json::to_vec(&Some(pyth_oracle::Price {
            price: 1.into(),
            conf: 2.into(),
            expo: 0,
            publish_time: 20,
        })).unwrap();
        global_register_degen(accounts(1).as_ref(), DegenType::PriceOracle { decimals: TARGET_DECIMAL });
        global_register_degen(accounts(2).as_ref(), DegenType::PythOracle { price_identifier: pyth_oracle::PriceIdentifier([1u8; 32]) });
        global_register_degen_oracle_config(DegenOracleConfig::PriceOracle(PriceOracleConfig { 
            oracle_id: "oracle_id".to_string(), 
            expire_ts: 60, 
            maximum_recency_duration_sec: 90, 
            maximum_staleness_duration_sec: 90
        }));
        global_register_degen_oracle_config(DegenOracleConfig::PythOracle(PythOracleConfig { 
            oracle_id: "oracle_id".to_string(), 
            expire_ts: 60, 
            pyth_price_valid_duration_sec: 60, 
        }));
        let mut degen1 = global_get_degen(accounts(1).as_ref());
        degen1.set_price(&cross_call_result1);
        println!("degen1: {:?}", degen1.get_price_info().stored_degen);
        global_set_degen(accounts(1).as_ref(), &degen1);
        let mut degen2 = global_get_degen(accounts(2).as_ref());
        degen2.set_price(&cross_call_result2);
        println!("degen2: {:?}", degen2.get_price_info().stored_degen);
        global_set_degen(accounts(2).as_ref(), &degen2);

        println!("degens: {:?}", pool.get_degens());
