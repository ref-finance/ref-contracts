...
impl DegenSwap {
    ...
    fn mul_degen(&self, amount: Balance, degen: Balance) -> Balance {
        (U384::from(amount) * U384::from(degen) / U384::from(PRECISION))
            .as_u128()
    }

    /// *
    fn div_degen(&self, amount: Balance, degen: Balance) -> Balance {
        (U384::from(amount) * U384::from(PRECISION) / U384::from(degen))
            .as_u128()
    }
    ...
    /// * Compute stable swap invariant (D) with degens applied to input balances
    pub fn compute_d_with_degens(&self, c_amounts: &Vec<Balance>) -> Option<U384> {
    ...
    pub fn compute_lp_amount_for_deposit(
        ...
        // Initial invariant
        let d_0 = self.compute_d_with_degens(old_c_amounts)?;
        ...
        let d_1 = self.compute_d_with_degens(&new_balances)?;
        if d_1 <= d_0 {
            None
        } else {
            ...
            let d_2 = self.compute_d_with_degens(&new_balances)?;
    ...
    pub fn swap_to(
        ...
        let dy = current_c_amounts_degen[token_out_idx].checked_sub(y)?.checked_sub(1).unwrap_or(0_u128); // * ? curve sub -1 just in case there were some rounding errors
        ...
        Some(SwapResult::new(
            ...
            self.div_degen(admin_fee, degen_out),
            self.div_degen(trade_fee, degen_out),
        ))
    }
}
