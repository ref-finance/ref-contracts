...
use crate::utils::{check_token_duplicates, check_string_duplicates};
...
impl Contract {
    ...
    /// Adds new "Simple Pool" with given uniform tokens and given fee.
    /// enable uniform_token_id instead of only nep-141 token_id in field `tokens`,
    /// limited to guardians access in case of unexpected use from misunderstanding
    #[payable]
    pub fn add_high_order_simple_pool(&mut self, tokens: Vec<AccountId>, fee: u32) -> u64 {
        ...
        check_string_duplicates(&tokens);
        let mut token_set = HashSet::<&String>::new();
        for token in &tokens {
            token_set.insert(token);

            // check if token is in valid format
            match TokenType::parse_token(token) {
                TokenType::Nep141 {token_id: _} => {
                ...
                TokenType::OuterMFT {token_contract_id: _, inner_id: _} => {
                    unimplemented!("OuterMFT not supported yet!");
                },
    ...
    pub fn add_liquidity(
        ...
        for i in 0..tokens.len() {
            match TokenType::parse_token(&tokens[i]) {
                TokenType::Nep141 {token_id} => {
                    deposits.withdraw(&token_id, amounts[i]);
                },
                TokenType::InnerMFT {pool_id} => {
    ...
    pub fn remove_liquidity(&mut self, pool_id: u64, shares: U128, min_amounts: Vec<U128>) {
        ...
        for i in 0..tokens.len() {
            match TokenType::parse_token(&tokens[i]) {
                TokenType::Nep141 {token_id} => {
                    deposits.deposit(&token_id, amounts[i]);
                },
                TokenType::InnerMFT {pool_id} => {
                    let token_id = format!(":{}", pool_id);
