...
fn test_hotzap_simple_pool() {
    ...
    let real_cost = view!(pool.predict_add_simple_liquidity(ETH_USDT, &vec![U128(out_eth_amount), U128(out_usdt_amount)])).unwrap_json::<(U128, Vec<U128>)>();
    println!("shares : {:?}", real_cost.0.0);
    println!("remain eth : {:?}", out_eth_amount - real_cost.1[0].0);
    println!("remain usdt : {:?}", out_usdt_amount - real_cost.1[1].0);
    ...
    println!("predict hot zap: {:?}", view!(pool.predict_hot_zap(
        ...
        1,
        Some(vec![U128(1814048647419868151852681u128), U128(907024323709934075926341u128)]),
        None
    )).unwrap_json::<(U128, HashMap<AccountId, U128>)>());
    ...
    let outcome = call!(
        ...
        token_dai.ft_transfer_call(
            ...
            json!({
                ...
                "pool_id": 1,
                "min_amounts": ["1814048647419868151852681", "907024323709934075926341"],
            }).to_string()
        ),
        1,
