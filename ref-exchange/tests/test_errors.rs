...
use near_sdk_sim::{init_simulator, call, to_yocto, ExecutionResult};
...
fn sim_stable_e83 () {
    ...
    let outcome = call!(
        owner,
        ex.stable_swap_ramp_amp(0, 0, U64(86400000000000))
    );
    assert_failure(outcome, "E83: invalid amp factor");

    let outcome = call!(
        owner,
        ex.stable_swap_ramp_amp(0, 1_000_001, U64(86400000000000))
    );
    assert_failure(outcome, "E83: invalid amp factor");
}
...
fn sim_stable_e84 () {
    ...
    let outcome = call!(
        owner,
        ex.stable_swap_ramp_amp(0, 1, U64(86400000000000))
    );
