...
const AMOUNT_IN_LIMIT: u128 = 20;
const TRANSFER_AMOUNT_LIMIT: u128 = 20;
...
fn get_test_token_amount(ctx: &mut OperationContext, operator: &Operator, token: &String) -> u128 {
    let token_contract = ctx.token_contract_account.get(token).unwrap();
...
fn do_direct_swap(ctx: &mut OperationContext, rng: &mut Pcg32, root: &UserAccount, operator: &Operator, pool :&ContractAccount<Exchange>, simple_pool_count: u64){
    let simple_pool_id = if simple_pool_count == 0 { 0 } else { rng.gen_range(0..simple_pool_count) };
    let simple_pool_info = view!(pool.get_pool(simple_pool_id)).unwrap_json::<PoolInfo>();
    ...
    let transfer_amount = to_yocto("20");
    ...
    loop {
        ...
        }else if test_token_in_amount == 0{
