...
fn failure_e41_when_deposit_reward_token() {
    ...
    let calldata = call!(
        root,
        token1.ft_transfer_call(to_va(farming_id()), U128(to_yocto("1")), None, format!("{}{}{}", "{\"farm_id\": \"", "swap@0#1", "\"}")),
        deposit = 1
    );
...
fn failure_e42_when_force_clean_farm() {
    ...
    let calldata = call!(
        root,
        token1.ft_transfer_call(to_va(farming_id()), U128(to_yocto("1")), None, format!("{}{}{}", "{\"farm_id\": \"", farm_id.clone(), "\"}")),
        deposit = 1
    );
    calldata.assert_success();
...
fn failure_e44() {
    ...
    let calldata = call!(
        root,
        token2.ft_transfer_call(to_va(farming_id()), U128(to_yocto("1")), None, format!("{}{}{}", "{\"farm_id\": \"", farm_id.clone(), "\"}")),
        deposit = 1
    );
    calldata.assert_success();
    let ex_status = format!("{:?}", calldata.promise_errors()[0].as_ref().unwrap().status());
