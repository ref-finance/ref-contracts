...
use ref_farming_v2::{HRSimpleFarmTerms, CDAccountInfo};
...
fn single_farm_cd_account() {
    ...
    call!(
        owner,
        farming.modify_cd_strategy_lock_time(0, to_nano(100))
    ).assert_success();

    call!(
        owner,
        farming.modify_cd_strategy_additional(0, 100)
    ).assert_success();

    call!(
        owner,
        farming.modify_cd_strategy_denominator(100)
    ).assert_success();
    ...
    out_come.assert_success();
    // println!("{:?}", out_come.promise_errors()[0].as_ref().unwrap().status());
    // let aa = view!(farming.get_number_of_user_cd_accounts(farmer1.valid_account_id())).unwrap_json::<u64>();
    // println!("{}", aa);
    // let bb = view!(farming.list_user_cd_account(farmer1.valid_account_id(), 0, 1)).unwrap_json::<Vec<CDAccountInfo>>();
    // println!("{:?}", bb);
    println!("<<----- Farmer1 staked liquidity at #{}, ts:{}.", 
    root.borrow_runtime().current_block().block_height, 
    root.borrow_runtime().current_block().block_timestamp);
